...
course = Course_T(courseID='ELT499', courseName="Database Management", numOfCredits=3, program_id=5,
                  courseType="Core")
course.save()

# CO
plolist = list(PLO_T.objects.filter(program=5))
...
faculties = []
faculties.append(Faculty_T.objects.get(pk=4501))
faculties.append(Faculty_T.objects.get(pk=4502))
faculties.append(Faculty_T.objects.get(pk=4503))

dept = Department_T.objects.get(pk="ENG")
program = Program_T.objects.get(pk=5)


def updatedatabase(d, sem):
    df = pd.read_excel("ELT499.xlsx", sheet_name="Marks")
