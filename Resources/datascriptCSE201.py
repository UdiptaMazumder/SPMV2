...
course = Course_T(courseID='CSE201', courseName="Discrete Mathematics", numOfCredits=3, program_id=1, courseType="Core")
course.save()
...
colist.append(CO_T(coNum="CO1", course=course, plo=plolist[5]))
colist.append(CO_T(coNum="CO2", course=course, plo=plolist[4]))
...
faculties = []
faculties.append(Faculty_T.objects.get(pk=4110))
faculties.append(Faculty_T.objects.get(pk=4111))
faculties.append(Faculty_T.objects.get(pk=4112))
...
def updatedatabase(d, sem, y):
    df = pd.read_excel("CSE201.xlsx", sheet_name="Marks")
