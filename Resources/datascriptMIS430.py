...
course = Course_T(courseID='MIS430', courseName="Data Communication and Networking", numOfCredits=4, program_id=4,
                  courseType="Core")
course.save()

# CO
plolist = list(PLO_T.objects.filter(program=4))
...
faculties = []
faculties.append(Faculty_T.objects.get(pk=4401))
faculties.append(Faculty_T.objects.get(pk=4402))
faculties.append(Faculty_T.objects.get(pk=4403))

dept = Department_T.objects.get(pk="MIS")
program = Program_T.objects.get(pk=4)


def updatedatabase(d, sem):
    df = pd.read_excel("MIS430.xlsx", sheet_name="Marks")
    ...
    for i in sections:
        ...
        section = Section_T(sectionNum=i, course=course, faculty=faculty, semester=sem)
        section.save()
    ...
    for i in data:
        ...
        reg = Registration_T(student=st, section=sectionlist[i[3] - 1], semester=sem)
        reg.save()
        reglist.append(reg)
