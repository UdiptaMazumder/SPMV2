...
course = Course_T(courseID='CSE203', courseName="Data Structure", numOfCredits=4, program_id=1, courseType="Core")
course.save()
...
colist.append(CO_T(coNum="CO2", course=course, plo=plolist[6]))
colist.append(CO_T(coNum="CO3", course=course, plo=plolist[9]))
colist.append(CO_T(coNum="CO4", course=course, plo=plolist[4]))
...
faculties = []
faculties.append(Faculty_T.objects.get(pk=4104))
faculties.append(Faculty_T.objects.get(pk=4105))
faculties.append(Faculty_T.objects.get(pk=4106))
...
def updatedatabase(d, sem, y):
    ...
    df = pd.read_excel("CSE203.xlsx", sheet_name="Marks")
